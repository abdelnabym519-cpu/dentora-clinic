...
<template>
  <div class="mx-auto max-w-7xl space-y-6 p-6" data-testid="prescriptions-page">
    <header>
      <h1 class="text-2xl font-semibold">Electronic Prescriptions</h1>
      <p class="text-sm text-gray-500">Create, issue and audit prescriptions for the selected clinic.</p>
    </header>

    <section class="rounded-xl border p-4 space-y-4" data-testid="prescription-create">
      <h2 class="font-semibold">New prescription</h2>
      <div class="relative">
      ...
      <div v-for="(item, index) in items" :key="index" class="grid gap-3 rounded border p-3 md:grid-cols-3">
        ...
        <button type="button" class="text-sm text-red-600" @click="removeItem(index)">Remove</button>
      </div>

      <div class="flex gap-3">
        <button type="button" class="rounded border px-3 py-2" @click="addItem">Add medication</button>
        <button
          type="button"
    ...
    <section class="space-y-3">
      ...
      <div v-for="rx in prescriptions" :key="rx.id" class="rounded-xl border p-4" :data-testid="`prescription-${rx.id}`">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <div>
            <strong>{{ rx.identifier }}</strong>
            <span class="ml-2 rounded bg-gray-100 px-2 py-1 text-xs" :data-testid="`prescription-status-${rx.id}`">{{ rx.status }}</span>
          </div>
          <div class="flex gap-2">
            <button v-if="rx.status === 'draft'" :data-testid="`issue-${rx.id}`" type="button" class="rounded bg-green-600 px-3 py-1 text-white" :disabled="busy" @click="issue(rx)">Issue</button>
            <button v-if="rx.status === 'draft'" type="button" class="rounded border px-3 py-1" :disabled="busy" @click="cancel(rx)">Cancel</button>
            <button v-if="rx.status === 'issued'" type="button" class="rounded border px-3 py-1" :disabled="busy" @click="voidRx(rx)">Void</button>
          </div>
        </div>
        ...
        <p v-if="rx.status !== 'draft'" class="mt-2 text-xs text-gray-500">Issued/terminal prescription content is immutable.</p>
      </div>
    </section>
  </div>
</template>
