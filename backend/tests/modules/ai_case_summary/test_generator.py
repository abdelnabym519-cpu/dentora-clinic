...
def _input():
    return {
        ...
        "sections": {
            ...
            "nerve": {"status": "invalid_or_stale", "data": None, "reason": "nerve_analysis_not_accepted"},
        },
    }
...
async def test_generator_requires_traceable_claims_and_complete_gaps() -> None:
    provider = FakeProvider({"claims": [{"claim_id": "C1", "text": "Tooth 16 is present.", "evidence_ids": ["E001"]}], "data_gaps": [{"section": "nerve", "status": "invalid_or_stale"}]})
    result = await generate_summary(provider=provider, model="test-model", llm_input=_input(), max_tokens=1000)
...
async def test_generator_rejects_unknown_evidence() -> None:
    provider = FakeProvider({"claims": [{"claim_id": "C1", "text": "Invented.", "evidence_ids": ["E999"]}], "data_gaps": [{"section": "nerve", "status": "invalid_or_stale"}]})
...
async def test_generator_rejects_omitted_missing_or_stale_data() -> None:
    provider = FakeProvider({"claims": [{"claim_id": "C1", "text": "Fact.", "evidence_ids": ["E001"]}], "data_gaps": []})
