...
from app.modules.prescriptions.use_cases import PrescriptionUseCases
...
class Repo:
    ...
    async def list(self, *, tenant_id, clinic_id, patient_id=None, status=None):
        return [
            rx
            for rx in self.rows.values()
            if rx.tenant_id == tenant_id
            and rx.clinic_id == clinic_id
            and (patient_id is None or rx.patient_id == patient_id)
            and (status is None or rx.status is status)
        ]
    ...
    async def audit(self, prescription_id, *, tenant_id, clinic_id):
        if await self.get(prescription_id, tenant_id=tenant_id, clinic_id=clinic_id):
            return [
                ...
                if event.get("prescription_id", prescription_id) == prescription_id
            ]
        return []
...
async def test_context_isolation_and_lifecycle_audit() -> None:
    ...
    repo = Repo()
    uc = PrescriptionUseCases(
        repo,
        Patients({(patient, clinic)}),
        clock=Clock(),
