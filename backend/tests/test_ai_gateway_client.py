...
class AiGatewayCredentialTests(unittest.IsolatedAsyncioTestCase):
    async def test_ai_disabled_blocks_gateway_credential(self):
        with SettingsPatch(
            ...
            with patch.object(
                ...
                with self.assertRaises(LicenseRejectedError) as ctx:
                    await license_manager.get_ai_gateway_credential()
    ...
    async def test_ai_enabled_returns_signed_lease(self):
        synthetic_token = "synthetic-payload.synthetic-signature"

        installation_id = "installation-test-001"

        fingerprint = "fingerprint-test-001"
        ...
        with SettingsPatch(
            ...
            with (
                ...
                result = await license_manager.get_ai_gateway_credential()
...
class AiGatewayProviderTests(unittest.IsolatedAsyncioTestCase):
    async def test_provider_uses_dynamic_gateway_credential(
        ...
        async def resolver():
            return "synthetic-payload.synthetic-signature"

        provider = OpenAIProvider(
            base_url=("https://gateway.example.test/ai/v1/"),
        ...
        self.assertEqual(
            captured["base_url"],
            ("https://gateway.example.test/ai/v1/"),
        )
    ...
    async def test_licensed_factory_uses_gateway_not_openai_key(
        ...
        with SettingsPatch(
            ...
            self.assertIsNotNone(provider._api_key_resolver)
    ...
    async def test_development_mode_keeps_direct_provider(
        ...
        with SettingsPatch(
            ...
            OPENAI_API_KEY=("development-test-key"),
        ):
            provider = get_provider("openai")
