...
| `dental_3d` | 0.6.0 | official | patients, odontogram, media | manual | yes | 2 | 0 | 0 | yes |
...
Dental 3D — 3D dentition preview on the patient summary with real mesh ingestion (STL/OBJ via the media module), a synthetic fallback and a non-clinical automatic tooth-segmentation foundation with dentist review, plus CBCT/DICOM ingestion and a replaceable, non-clinical real nerve-inference boundary with native-coordinate findings and explicit unavailable/failure states.
