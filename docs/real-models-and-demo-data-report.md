...
**Defect 3 — the dev server rejected the preview host with a 403.** Vite answers unknown `Host` headers with a hard block, so any reverse-proxied preview failed before Nuxt ever rendered:

```
$ curl -H "Host: 3000-<sandbox>.e2b.app" localhost:3000/login   -> HTTP 403
Blocked request. This host ("3000-<sandbox>.e2b.app") is not allowed.
To allow this host, add "3000-<sandbox>.e2b.app" to `server.allowedHosts` in vite.config.js.
```

Fix: `vite.server.allowedHosts` is now populated from a new `NUXT_DEV_ALLOWED_HOSTS` environment variable (comma-separated; a leading dot allows a whole domain). No sandbox-specific value is baked into the repository, and leaving the variable unset preserves Vite's default localhost-only allowlist.

Backend CORS was already correct once `ALLOWED_ORIGINS` included the preview origins:

```
$ curl -X OPTIONS .../api/v1/auth/login -H "Origin: https://3000-<sandbox>.e2b.app"
access-control-allow-origin: https://3000-<sandbox>.e2b.app
...
And through the running UI, server-side rendered with a real session cookie — including via the reverse-proxy `Host` header, which is how a browser reaches it:

```
GET /patients/e2eebc99-9c0b-4ef8-bb6d-6bb9bd380a62  -> HTTP 200, 87,072 bytes
  (both on localhost:3000 and with Host: 3000-<sandbox>.e2b.app)
SSR HTML contains: "Layla", "Adel", "SYNTHETIC" (x3)
GET /login via the preview Host -> HTTP 200, 0 host-block markers
```
...
If a card is missing, the cause is almost always layer registration — check that `frontend/modules.json` lists 29 layers and that the dev server logs no `Cannot extend config` warnings. If the whole app returns `403 Blocked request. This host is not allowed`, the dev server is behind a proxy whose hostname is not in `NUXT_DEV_ALLOWED_HOSTS`.
...
# DENTORA_FRONTEND_ROOT must be exported before the BACKEND starts, so its
# layer sync rewrites frontend/modules.json (29 layers).
export DENTORA_FRONTEND_ROOT="$PWD/frontend"

cd frontend && npm run dev
# Only needed when the dev server sits behind a reverse proxy / tunnel:
#   NUXT_DEV_ALLOWED_HOSTS=".your-proxy-domain" npm run dev
```
...
- `frontend/nuxt.config.ts` — (a) resolve container-absolute layer paths under the frontend root when absent on disk; (b) populate `vite.server.allowedHosts` from `NUXT_DEV_ALLOWED_HOSTS` so reverse-proxied previews are not rejected with a 403.
