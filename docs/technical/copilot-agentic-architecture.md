...
- `ollama_provider.py` — **local, self-hosted provider.** `OllamaProvider` talks to an Ollama server's native streaming `POST /api/chat` NDJSON endpoint (no cloud, no API key): text deltas, `tool_calls` (same `.`↔`-` name bijection), `role:"tool"` results, and usage from the final `done:true` object (`prompt_eval_count`/`eval_count`). Selected with `provider="ollama"` / `COPILOT_PROVIDER_DEFAULT=ollama`; configured via `OLLAMA_BASE_URL` + `OLLAMA_MODEL`. It implements the same `Provider` protocol, so orchestrator/redaction/budget/audit are untouched.
- `factory.py` — `get_provider(name: str) -> Provider` resolving `"openai"` (cloud) and `"ollama"` (fully local); raises a clear "unsupported provider" error for anything else. `default_model_for(provider)` keeps the model id coherent when a clinic switches provider. The orchestrator/bridge passes the chosen `model` string and OpenAI-shaped tool schemas (Ollama reuses the same tool envelope).
